...
BPNN *net;
{
  ...
  sscanf(NAME(img), "%[^_]_%[^_]_%[^_]_%[^_]_%d.%[^_]", userid, head,
         expression, eyes, &scale, photo);

  // char poses[4][10] = {"left", "right", "up", "straight"};
  //
  // for (int i = 0; i < 4; i++) {
  //   if (!strcmp(head, poses[i])) {
  //     net->target[i + 1] = TARGET_HIGH;
  //   } else {
  ...
  // }
  char expressions[4][10] = {"sad", "neutral", "happy", "angry"};

  for (int i = 0; i < 4; i++) {
    if (!strcmp(expression, expressions[i])) {
      net->target[i + 1] = TARGET_HIGH;
    } else {
      net->target[i + 1] = TARGET_LOW;
    }
...
load_input_with_image(img, net) IMAGE *img;
BPNN *net;
{
  double *units;
