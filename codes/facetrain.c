...
char *netname;
{
  ...
  if ((net = bpnn_read(netname)) == NULL) {
    if (train_n > 0) {
      ...
      /* bthom ===========================
        make a net with:
          imgsize inputs, 4 hidden units, and 4 output units
          */
      net = bpnn_create(imgsize, 4, 4); // from (4,1) to (4,4)
    } else {
...
performance_on_imagelist(net, il, list_errors) BPNN *net;
IMAGELIST *il;
int list_errors;
{
  double err, val;
...
evaluate_performance(net, err) BPNN *net;
double *err;
{
  double delta, total_error = 0.0;
  int correct_class = -1, predicted_class = -1;

  delta = net->target[1] - net->output_units[1];

  for (int i = 1; i <= 4; i++) { // start at 1 since the arrays are 1-indexed
    delta = net->target[i] - net->output_units[i];
    total_error += (0.5 * delta * delta);
    ...
    // find the unit with the highest activation
    if (predicted_class == -1 ||
        net->output_units[i] > net->output_units[predicted_class]) {
      predicted_class = i;
    }
  }
  ...
  *err = total_error;
  ...
  return (predicted_class == correct_class) ? 1 : 0;
}

printusage(prog) char *prog;
{
  printf("USAGE: %s\n", prog);
